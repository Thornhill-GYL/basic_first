...
void Uart_Init (UARTn_e uratn, uint32 baud)
{
    ...
    //计算波特率，串口0-5使用bus时钟   
    sysclk = (uint32)(fastperipheral_clk_khz*1000);                          //bus时钟(是内核时钟的一半)
...
//函数名: uart_pendchar                                                        
//功  能: 接收一个字节                                                        
//参  数: uratn:模块名如：UART_0 
//        ch: 保存接收数据指针
//返  回: 无                                                              
//简  例: uart_pend_char (UART_4,a[]) 将接收到的数据保存在a数组里                                
//-------------------------------------------------------------------------*
char UART_Pend_Char (UARTn_e uratn, char *ch)
{
    u32 i = 0;
