...
#if USE_HARDWARE_SPI
  spi_init(LCD_SPIn,LCD_OTH_PCSn | LCD_DC_PCSn,MASTER,100*1000*1000);//12500*1000);
#else
...
void LCD_init(unsigned char direction)
{
  ...
  if(direction)
    LCD_write_data(0xE8); //C8	   //48 68竖屏//28 E8 横屏
  else
...
{
  int i,j;
  LCD_SetPos(0,319,0,239-IMAGE_CORRECT);
  for(i=IMAGE_CORRECT;i < CAMERA_H - IMAGE_CORRECT_NEAR ;i++)
  {
...
//      else if(i == Cross.Str_Cross_test_hang)
//        write_word(Purple);
      else if(IMG_PIXEL(j,i)==1)
        write_word(Black);
      else
